...
std::ostream& operator<<(std::ostream& stream, const geometry_msgs::msg::Point position)
{
  stream << "{x: " << position.x << ", y: " << position.y << ", z: " << position.z << "}";
  return stream;
}

std::ostream& operator<<(std::ostream& stream, const geometry_msgs::msg::Quaternion orientation)
{
  stream << "{x: " << orientation.x << ", y: " << orientation.y << ", z: " << orientation.z <<  ", w: " << orientation.w << "}" << std::endl;
  return stream;
}

std::ostream& operator<<(std::ostream& stream, const geometry_msgs::msg::Pose pose)
{
  stream << "point: " << pose.position << " orientation: " << pose.orientation << std::endl;
  return stream;
}
...
int main(int argc, char ** argv)
{
  ...
  while (rclcpp::ok()) {
    ...
    bool over_line;
    if (obj->getCurrentLaneID(current_id)) RCLCPP_INFO_STREAM(obj->get_logger(), "getCurrentLaneID:" << current_id);
    ...
    RCLCPP_INFO_STREAM(obj->get_logger(), "isInLane:" << obj->isInLane());  // base_link is in lane or not
    ...
    RCLCPP_INFO_STREAM(obj->get_logger(), "WillLaneChange" << obj->willLaneChange());  // temporary implementation
    ...
    if (obj->finishNPCLaneChange("car_uturn_0", &finish_lane_change)) {
      RCLCPP_INFO_STREAM(obj->get_logger(), "car_uturn_0: finish lane change?" << finish_lane_change);
    }
    bool finish_vel_change;
    if (obj->finishNPCVelocityChange("car_uturn_0", &finish_vel_change)) {
      RCLCPP_INFO_STREAM(obj->get_logger(), "car_uturn_0: finish velocity change?" << finish_vel_change);
    }
