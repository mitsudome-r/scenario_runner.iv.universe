...
{
  rclcpp::init(argc, argv, "scenario_api");
  ScenarioAPI obj;

  /* sample code */
  obj.setEgoCarName("EgoCar");
  obj.waitAutowareInitialize();  // wait for initialization of autoware
  geometry_msgs::msg::msg::Pose ego_car_start_pose = obj.genPoseROS(3820.9, 73772.9, 0.0, 3.58);
  obj.sendStartPoint(ego_car_start_pose);  // send start point(x, y, z, yaw)
  obj.setFrameId("/initialpose", ego_car_start_pose);
  geometry_msgs::msg::msg::Pose ego_car_goal_pose = obj.genPoseROS(3714.2, 73741.65, 0.0, 2.01);
  obj.sendGoalPoint("EgoCar", ego_car_goal_pose, true, "Front");  // send goal point(name, pose)
  geometry_msgs::msg::msg::Pose ego_car_checkpoint_pose = obj.genPoseROS(3717.9, 73684.4, 0.0, 2.01);
  obj.sendCheckPoint(
    "EgoCar",
  ...
  // const geometry_msgs::msg::msg::Pose pose, const double velocity);
  geometry_msgs::msg::msg::Pose ped0_pose = obj.genPoseROS(3715.7, 73773.87, 0.0, 0.474);
  obj.addNPC("pedestrian", "ped0", ped0_pose, 0.2);
  obj.changeNPCVelocityWithAccel("ped0", 3, 1.0);

  geometry_msgs::msg::msg::Pose bicycle0_pose = obj.genPoseROS(3819.2, 73799.2, 0.0, -1.096);
  obj.addNPC("bicycle", "bicycle0", bicycle0_pose, 5.0);

  geometry_msgs::msg::msg::Pose car0_pose = obj.genPoseROS(3819.2, 73799.2, 0.0, -1.096);
  obj.addNPC("car", "car0", car0_pose, 0.0);

  // generate NPC to change lane(left, right)
  geometry_msgs::msg::msg::Pose car_lc_0_pose = obj.genPoseROS(3701.0, 73764.7, 0.0, 0.467);
  obj.addNPC("car", "car_lc_0", car_lc_0_pose, 1.0);
  obj.changeNPCLaneChangeRight("car_lc_0");
  geometry_msgs::msg::msg::Pose car_lc_1_pose = obj.genPoseROS(3702.6, 73762.1, 0.0, 0.467);
  ...
  geometry_msgs::msg::msg::Pose car_lc_2_pose = obj.genPoseROS(3724.5, 73773.2, 0.0, 0.467);
  ...
  geometry_msgs::msg::msg::Pose car_acc_tr_0_pose = obj.genPoseROS(3702.6, 73762.1, 0.0, 0.467);
  ...
  geometry_msgs::msg::msg::Pose car_uturn_0_pose = obj.genPoseROS(3746.8, 73685.0, 0.0, -2.67);
  ...
  geometry_msgs::msg::msg::Pose car_fr_1_start_pose =
    obj.genPoseROS(3751.0, 73786.8, 0.0, 0.0, 0.0, 0.2361, 0.9717);
  geometry_msgs::msg::msg::Pose car_fr_1_goal_pose =
    obj.genPoseROS(3721.4, 73721.8, 0.0, 0.0, 0.0, -0.9678, 0.2516);
  geometry_msgs::msg::msg::Pose car_fr_1_checkpoint_pose =
    obj.genPoseROS(3776.8, 73718.3, 0.0, 0.0, 0.0, 0.8536, 0.5209);
  ...
  if (obj.getTrafficLightColor(34806, &color, false)) {
    RCLCPP_INFO_STREAM(get_logger(), "traffic:34806 color(RED) is " << color);
  } else {
    RCLCPP_INFO_STREAM(get_logger(), "failed to get traffic:34806 color");
  }
  obj.updateState();
  rclcpp::Rate(0.3).sleep();  //3.0s sleep
  if (obj.getTrafficLightArrow(34806, &arrows, false)) {
    RCLCPP_INFO_STREAM(get_logger(), "traffic:34806 arrow size(NULL) is " << arrows.size());
  }
  obj.setTrafficLightArrow(34806, "left", false);
  obj.setTrafficLightArrow(34806, "up", false);
  if (obj.getTrafficLightArrow(34806, &arrows, false)) {
    RCLCPP_INFO_STREAM(get_logger(), "traffic:34806 arrow size(2) is " << arrows.size());
    for (const auto arrow : arrows) {
      RCLCPP_INFO_STREAM(get_logger(), "traffic:34806 arrow (left, up)) is " << arrow);
    }
  }
  obj.updateState();
  rclcpp::Rate(0.3).sleep();  //3.0s sleep
  ...
  if (obj.getTrafficLightArrow(34806, &arrows, false)) {
    RCLCPP_INFO_STREAM(get_logger(), "traffic:34806 arrow size(0) is " << arrows.size());
    for (const auto arrow : arrows) {
      RCLCPP_INFO_STREAM(get_logger(), "traffic:34806 arrow (null)) is " << arrow);
    }
  }
  obj.updateState();
  rclcpp::Rate(0.3).sleep();  //3.0s sleep
  ...
  while (rclcpp::ok()) {
    ...
    RCLCPP_INFO_STREAM(get_logger(), "velocity:" << obj.getVelocity());
    RCLCPP_INFO_STREAM(get_logger(), "accel:" << obj.getAccel());
    RCLCPP_INFO_STREAM(get_logger(), "jerk:" << obj.getJerk());
    RCLCPP_INFO_STREAM(get_logger(), "total move distance:" << obj.getMoveDistance());
    RCLCPP_INFO_STREAM(get_logger(), 
    ...
    bool over_line;
    if (obj.getCurrentLaneID(current_id)) RCLCPP_INFO_STREAM(get_logger(), "getCurrentLaneID:" << current_id);
    if (obj.getDistanceToTrafficLight(34806, dist1)) {
      RCLCPP_INFO_STREAM(get_logger(), "dist to traffic light: 34806 is " << dist1);
    }
    if (obj.getDistanceToTrafficLine(34806, dist2)) {
      RCLCPP_INFO_STREAM(get_logger(), "dist to traffic light line: 34806 is " << dist2);
    } else if (obj.getDistancefromCenterLine(dist3))
      RCLCPP_INFO_STREAM(get_logger(), "getDistancefromCenterLine:" << dist3);
    if (obj.checkOverTrafficLine(34806, over_line)) {
      RCLCPP_INFO_STREAM(get_logger(), "over stop line (34806)? :" << over_line);
    }
    RCLCPP_INFO_STREAM(get_logger(), "isInLane:" << obj.isInLane());  // base_link is in lane or not
    RCLCPP_INFO_STREAM(get_logger(), "LeftBlinker:" << obj.getLeftBlinker());
    RCLCPP_INFO_STREAM(get_logger(), "RightBlinker:" << obj.getRightBlinker());
    RCLCPP_INFO_STREAM(get_logger(), "WillLaneChange" << obj.willLaneChange());  // temporary implementation
    Pose2D p = obj.getCurrentPose();
    RCLCPP_INFO_STREAM(get_logger(), "selfpose:" << p.x << "," << p.y << "," << p.yaw);
    geometry_msgs::msg::msg::PoseStamped p2 = obj.getCurrentPoseRos();
    RCLCPP_INFO_STREAM(get_logger(), "selfpose:" << p2.pose.position << "," << p2.pose.orientation);
    double dist_npc;
    if (obj.calcDistToNPC(dist_npc, "car0")) {
      RCLCPP_INFO_STREAM(get_logger(), "distance to npc:car0:" << dist_npc);
    }
    double npc_vel;
    double npc_acc;
    if (obj.getNPCVelocity("car_acc_tr_0", &npc_vel)) {
      RCLCPP_INFO_STREAM(get_logger(), "velocity of npc:car_acc_tr_0:" << npc_vel);
    }
    if (obj.getNPCAccel("car_acc_tr_0", &npc_acc)) {
      RCLCPP_INFO_STREAM(get_logger(), "accel of npc:car_acc_tr_0:" << npc_acc);
    }
    RCLCPP_INFO_STREAM(get_logger(), 
    ...
    if (obj.finishNPCLaneChange("car_uturn_0", &finish_lane_change)) {
      RCLCPP_INFO_STREAM(get_logger(), "car_uturn_0: finish lane change?" << finish_lane_change);
    }
    bool finish_vel_change;
    if (obj.finishNPCVelocityChange("car_uturn_0", &finish_vel_change)) {
      RCLCPP_INFO_STREAM(get_logger(), "car_uturn_0: finish velocity change?" << finish_vel_change);
    }
  ...
  RCLCPP_INFO_STREAM(get_logger(), "scenario end");
  return 0;
};
